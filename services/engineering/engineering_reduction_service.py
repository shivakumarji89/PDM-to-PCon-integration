...
Grouping, classification and materialisation are strictly read-only: they
never mutate Engineering, members, or assignments, and the engine performs no
generation or synchronization. The single exception is
:meth:`EngineeringReductionService.apply_validated_reduction`, the enforcing
apply step, which writes ``member.reduced_article`` (and nothing else) through
the member service.

It also exposes the LEGACY CHECK for the candidate families it produces:
:meth:`EngineeringReductionService.validate_article_sets` takes each
materialised :class:`~models.article_set.ArticleSet` - the candidate family
this engine already derived - and proves or rejects it against the real legacy
``dbo.ProductsList`` boundary via
:class:`~services.engineering.pdm_family_reduction_service.PDMFamilyReductionService`.
The grouping itself is never taken from that validator: it validates what this
engine decided, it does not decide it. Validation reads ProductIds and
AttributeValueIds only - it never inspects article characters, so the article
suffix generated after the ``.`` is untouched by it.

``apply_validated_reduction`` then collapses ONLY the confirmed families: a
candidate asserting a cross-Product base that ``ProductsList`` did not confirm
stays expanded, and never silently passes for want of a check.
"""
...
@dataclass(frozen=True)
class PropertyClass:
    """Articles of ONE ProductRange that share an identical PROPERTY SIGNATURE.
    ...
    single desk (A side only) - form separate classes.

    The class is also scoped to one ``ProductRange``. A range is where PDM keeps
    the order-code template (``ProductRange.OrderCodeFormatString``), and it is
    the only scope its product filter (``dbo.ProductsList``) can answer in, so a
    class spanning ranges has no single base article to derive: its members'
    codes come from different templates and are commonly of different lengths.
    ...
    #: The snapshot's ProductRange NAME every article in this class belongs to
    #: ("" when the snapshot carries no range for these products).
    product_range: str = ""
    id: str = field(default_factory=lambda: uuid4().hex)
...
class EngineeringReductionService(BaseService):
    ...
    def classify_by_properties(
        ...
        Classification is ProductRange-scoped FROM THE START: the range is part
        of the class key, not a slice applied afterwards. PDM keeps the
        order-code template per range and filters products per range, so two
        ranges that happen to share a property structure are still two families
        with two base articles. Grouping them first and splitting later let one
        class mix article codes of different lengths from different templates,
        which made its derived base length meaningless for every member.
        ...
        # Key: (ProductRange, property signature) - first-seen order.
    ...
    def materialize_article_sets(
        ...
        for pc in classes:
            ...
            # Base length. $BAN ends at the earliest head position that this
            # set actually CONFIGURES. A head property whose value is the same
            # for every product in the set does not configure anything here: it
            # is set identity and belongs in the base (the PDM $BAN rule - see
            # docs/SKU_Config_Decode_Findings.md). Counting those constant
            # properties as configurable cut the base back to the first head
            # position any member merely CARRIES, which for a Nevi desk family
            # left "DWE" where PDM's own handbook records "DWE36A".
            #
            # Values are compared by normalised NAME, so PDM's duplicate value
            # rows (one value defined once per sub-series) never look like
            # variation. Two genuinely distinct codes sharing a name can only
            # make the base look constant, and the shared-prefix cap below then
            # shortens it again - the error can never lengthen a base.
            varying_head_positions = [
                head_layout[str(a.id)]["position"]
                for a in properties
                if str(a.id) in head_layout
                and head_layout[str(a.id)].get("width", 0)
                and len({normalize_value_name(v.value) for v in a.values}) > 1
            ]
            codes = [code_of.get(a, "") for a in article_ids]
            if varying_head_positions:
                base_length = max(min(varying_head_positions), 0)
            else:
                # Nothing in the head varies, so the whole head is the base.
                # The head is everything before the "." delimiter; with no
                # delimiter to go by, fall back to the tail-only rule (full
                # code length minus the coded tail widths).
                heads = [c.find(".") for c in codes if c and "." in c]
                if heads:
                    base_length = max(min(heads), 0)
                else:
                    code_len = max((len(c) for c in codes), default=0)
                    config_width = sum(
