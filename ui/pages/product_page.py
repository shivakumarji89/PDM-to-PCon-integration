...
class ProductPage(BasePage):
    ...
    def _build_context(self) -> QWidget:
        """Central product-level Repository Workspace.

        This replaces the old Engineering Context/Snapshot information panel.
        The workspace is the common place for repository linkage and product
        identity. PDM remains available as a live update source; repository
        selection establishes the baseline for existing-series maintenance.
        """
        ...
        self._open_repository_btn.setToolTip(
            "Select the repository/workspace root used for existing product discovery"
        )
        ...
        sync_section = QGroupBox("Data Sources", box)
        sync_layout = QVBoxLayout(sync_section)
        sync_layout.setContentsMargins(8, 6, 8, 8)
        ...
        sync_layout.addWidget(source_note)

        self._check_pdm_btn = QPushButton("Check PDM Changes", sync_section)
        self._check_pdm_btn.setToolTip(
            "Compare the current product workspace with available PDM data"
        )
        self._check_pdm_btn.setEnabled(False)
        sync_layout.addWidget(self._check_pdm_btn)
    ...
    def _on_open_repository(self) -> None:
        ...
        This first integration step deliberately establishes the connection only.
        Repository discovery/loading is added on top of this stable UI contract,
        rather than mixing reverse-loading logic into the existing PDM pipeline.
        """
    ...
    def _refresh_display(self, product, duration, warnings) -> None:
        # Keep product identity centralized beside repository controls. Detailed
        # load diagnostics remain available through the Activity panel instead
        # of consuming permanent workspace space.
